...
impl ICrdClient<
    ...
    SecretClaim
> for CrdClient {
    async fn get_config_stores(&self, params: &ListParams, namespace: &str) -> Result<ObjectList<ConfigurationStore>, Error> {
        Api::<ConfigurationStore>::namespaced((*self.client).clone(), namespace)
            .list(&params).await.map_err(Error::KubeError)
    }
    async fn get_cluster_config_stores(&self, params: &ListParams) -> Result<ObjectList<ClusterConfigurationStore>, Error> {
        Api::<ClusterConfigurationStore>::all((*self.client).clone())
            .list(&params).await.map_err(Error::KubeError)
    }
    async fn get_cluster_config_store(&self, name: &str) -> Result<ClusterConfigurationStore, Error> {
    ...
    async fn get_config_store(&self, name: &str, namespace: &str) -> Result<ConfigurationStore, Error> {
    ...
    async fn get_config_map_claims(&self, params: &ListParams, namespace: &str) -> Result<ObjectList<ConfigMapClaim>, Error> {
        let cmc_api = Api::<ConfigMapClaim>::namespaced((*self.client).clone(), namespace);
        cmc_api.list(&params).await.map_err(Error::KubeError)
    }
    async fn get_config_map_claim(&self, name: &str, namespace: &str) -> Result<ConfigMapClaim, Error> {
    ...
    async fn get_secret_claims(&self, params: &ListParams, namespace: &str) -> Result<ObjectList<SecretClaim>, Error> {
