...
mod tests {
    ...
    use crate::controller::v1alpha1::crd::claim::{
        ClaimConfigurationStoreRef, SupportedConfigurationStoreResourceType,
    };
    use crate::controller::v1alpha1::crd::configuration_store::ClusterConfigurationStore;
    use crate::controller::utils::config::Config as AppConfig;
    ...
    async fn test_successful_config_store_data_resolution(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        ...
        fixture
            .prepare_single_config_store_claim_scenario(
                store_name,
                vec![MockConfig::success_with_body("{\"asd\": 1}")],
            )
            .await;
        ...
        let store = ctx
            .v1alpha1
            .get_config_store(format!("{}-store", store_name).as_str(), namespace)
            .await?;
    ...
    async fn test_successful_config_store_common_parameter(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        fixture
            .prepare_single_config_store_claim_scenario(
                store_name,
                vec![MockConfig::query_params_response(
                    query_params,
                    "{\"test\": 1}",
                )],
            )
    ...
    async fn test_successful_cluster_config_store_data_resolution(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        fixture
            .prepare_single_cluster_config_store_claim_scenario(
                store_name,
                vec![MockConfig::success_with_body("{\"asd\": 1}")],
            )
            .await;
        ...
        let store = ctx
            .v1alpha1
            .get_cluster_config_store(format!("{}-store", store_name).as_str())
            .await?;
    ...
    async fn test_client_error_config_store_data_resolution(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        fixture
            .prepare_single_config_store_claim_scenario(
                store_name,
                vec![MockConfig::not_found_with_body(
                    "{\"error\": \"Not found\"}",
                )],
            )
            .await;
    ...
    async fn test_server_error_config_store_data_resolution(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        fixture
            .prepare_single_config_store_claim_scenario(
                store_name,
                vec![MockConfig::internal_server_error_with_body(
    ...
    async fn test_config_store_returns_data_depending_on_params(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        let config1 = config_store
            .get_config(Some(params1), None)
            .await
            .expect("Configuration based on test1 param couldn't be retrieved");

        let mut params2 = HashMap::new();
        params2.insert(String::from("test1"), String::from("value2"));
        let config2 = config_store
            .get_config(Some(params2), None)
            .await
            .expect("Configuration based on test1 param couldn't be retrieved");
    ...
    async fn test_basic_reconcilation(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
        ...
        fixture
            .prepare_single_config_store_claim_scenario(
                store_name,
                vec![MockConfig::success_with_body(
                    "{\"dbConfig\": { \"host\": \"test_host\", logLevels: [\"INFO\", \"DEBUG\"]} }",
                )],
            )
            .await;
        ...
        let claim = ctx
            .v1alpha1
            .get_config_map_claim(format!("{}-cmc", store_name).as_str(), namespace)
            .await
            .expect("Config Map Claim could not be found");
        let secret_claim = ctx
            .v1alpha1
            .get_secret_claim(format!("{}-sc", store_name).as_str(), namespace)
            .await
            .expect("Secret Claim could not be found");
        ...
        let mut config_map = ctx
            .v1alpha1
            .get_config_map(format!("{}-cmc", store_name).as_str(), namespace)
            .await
            .expect("Config map was not reconciled properly");
        let mut secret_map = ctx
            .v1alpha1
            .get_secret(format!("{}-sc", store_name).as_str(), namespace)
            .await
            .expect("Secret was not reconciled properly");
...
"#
        ...
        assert_eq!(
            data3,
            r#"dbConfig.host=test_host
dbConfig.logLevels.0=INFO
dbConfig.logLevels.1=DEBUG"#
        );
        assert_eq!(
            data4,
            r#"DB__CONFIG_HOST=test_host
...
DB__CONFIG_LOG__LEVELS_1=DEBUG"#
    ...
    async fn test_config_files_with_merging_reconcilation(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
...
DB__CONFIG_TIMEOUT=1000"#
    ...
    async fn test_basic_reconcilation_with_cluster_config_store(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
...
DB__CONFIG_LOG__LEVELS_1=DEBUG"#
    ...
    async fn test_config_files_with_merging_reconcilation_with_cluster_config_store(
        ctx: Arc<Context>,
        fixture: &mut ControllerFixtures,
    ) -> Result<String, Error> {
        // Verify `ClusterConfigurationStore` CRD is deployed
